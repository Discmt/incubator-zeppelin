...
function start() {
  ...
  nohup nice -n $ZEPPELIN_NICENESS $ZEPPELIN_RUNNER -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=8082 $JAVA_OPTS -cp $CLASSPATH $ZEPPELIN_MAIN >> "${ZEPPELIN_OUTFILE}" 2>&1 < /dev/null &
  pid=$!
  if [[ -z "${pid}" ]]; then
